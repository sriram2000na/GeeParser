...
class Assignment(object):
    ...
    def solve(self):
        printing = {'float': 'number', 'bool': 'boolean'}
        value = self.right.solve()
        for scope in stack:
            if self.left.name in scope:
                if scope[self.left.name][1] != type(value).__name__:
                    print(self.left.name,
                          scope[self.left.name][1], type(value).__name__)
                    message = f'Type Error: {printing[scope[self.left.name][1]]+""} = {printing[type(value).__name__]}!'
                    writeandExit(message)
                    return
                scope[self.left.name] = value, scope[self.left.name][1]
                return
        stack[-1][self.left.name] = value, type(value).__name__
...
class BinaryExpr(Expression):
    ...
    def solve(self):
        printing = {'float': 'number', 'bool': 'boolean'}
        if type(self.left.solve()) != type(self.right.solve()):
            message = f'Type Error: {printing[type(self.left.solve())]} = {printing[type(self.right.solve())]}!'
            # writeandExit(message)
...
class Identifier(Expression):
    ...
    def solve(self):
        ...
        # writeandExit(
        #     f'Type Error: {self.name} is referenced before being defined!')
        error(f'Uninitialized variable: {self.name}')
